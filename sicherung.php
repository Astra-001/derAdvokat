...
if($_POST['darlehen'])
{
	...
	/*if ($_POST['belastung']=='')
	{
	    $msg_belastung = "Bitte geben Sie - Belastungen und Voreintragungen - ein";
	    $smarty->assign('msg_voreintragungen', $msg_belastung);
	    $smarty->assign('msg_belastung', $msg_belastung);
	}*/
	if ($_POST['blatt']=='')
	{
	    $msg_blatt = "Bitte geben Sie - Blatt - ein";
	    $smarty->assign('msg_dar', $msg_blatt);
	    $smarty->assign('msg_blatt', $msg_blatt);
	}
	if ($_POST['grundbuch']=='')
	{
	    $msg_grundbuch = "Bitte geben Sie - eingetragen im Grundbuch von - ein";
	    $smarty->assign('msg_dar', $msg_grundbuch);
	    $smarty->assign('msg_grundbuch', $msg_grundbuch);
	}
	if ($_POST['gemarkung']=='')
	{
	    $msg_gemarkung = "Bitte geben Sie - auf Gemarkung - ein";
	    $smarty->assign('msg_dar', $msg_gemarkung);
	    $smarty->assign('msg_gemarkung', $msg_gemarkung);
	}
	if ($_POST['grundstuck']=='')
	{
	    $msg_grundstuck = "Bitte geben Sie - Grundstücke FIStNr. - ein";
	    $smarty->assign('msg_dar', $msg_grundstuck);
	    $smarty->assign('msg_grundstuck', $msg_grundstuck);
	}
	/*if ($_POST['dauer']=='')
	{
	...
	if (!$_POST['person'])
	{
	    $msg_person = "Bitte treffen Sie eine Auswahl - Sicherungsgeber oder Sicherungsnehmer - ?";
	...
	if(!isset($msg_person) AND !isset($msg_name) AND !isset($msg_vorname) AND !isset($msg_strasse) AND !isset($msg_plz) AND !isset($msg_ort) AND !isset($msg_beginn) AND !isset($msg_summe) AND !isset($msg_zins) AND !isset($msg_dauer) AND !isset($msg_grundstuck) AND !isset($msg_gemarkung) AND !isset($msg_grundbuch) AND !isset($msg_blatt))
	{
		switch(1)#$_POST['document']
		{
			case 1: require_once("rtf/sicherung_rtf.php");break;
...
$smarty->assign('contentTemplate', 'sicherung.tpl');