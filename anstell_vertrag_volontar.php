...
if($_POST['einstellung_allg'])
{
    ...
    if ($_POST['gehalt']=='')
	{
	    $msg_gehalt = "Bitte geben Sie die - Verg&uuml;tungssumme - ein";
	...
	if(!isset($msg_name) AND !isset($msg_vorname) AND !isset($msg_strasse) AND !isset($msg_plz) AND !isset($msg_ort) AND !isset($msg_beginn) AND !isset($msg_bereich) AND !isset($msg_gehalt))
	{
		require_once("rtf/anstell_vertrag_volontar_rtf.php");
	}
}
else
{
...
$smarty->assign('contentTemplate', 'anstell_vertrag_volontar.tpl');